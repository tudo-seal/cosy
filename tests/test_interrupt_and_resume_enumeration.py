...
def _recomputed(rule: RHSRule) -> set:
    ...
    for term in _TERMS:
        existing["a"].add(term)
        trees.update(space._generate_new_trees(rule, existing, None, None, ("a", term)))  # noqa: SLF001
    return trees
...
def _resumed(rule: RHSRule) -> set:
    ...
    trees.update(space._generate_new_trees(rule, existing, None, None, None, progress))  # noqa: SLF001
    for term in _TERMS:
        existing["a"].add(term)
        trees.update(space._generate_new_trees(rule, existing, None, None, ("a", term), progress))  # noqa: SLF001
    return trees
...
def test_interrupted_call_resumes_without_losing_trees(mixed_slot_rule: RHSRule) -> None:
    ...
    existing: dict = {"a": set()}
    progress: _RuleProgress = _RuleProgress()
    resumed.update(space._generate_new_trees(mixed_slot_rule, existing, None, None, None, progress))  # noqa: SLF001
    for index, term in enumerate(_TERMS):
        existing["a"].add(term)
        # interrupt
        max_count = 1 if index == 2 else None
        resumed.update(
            space._generate_new_trees(mixed_slot_rule, existing, None, max_count, ("a", term), progress)  # noqa: SLF001
        )
